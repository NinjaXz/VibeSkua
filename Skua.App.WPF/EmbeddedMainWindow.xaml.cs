...
public partial class EmbeddedMainWindow : Window
{
    ...
    private IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
    {
        ...
        else if (msg == WM_SKUA_ACCEPT_QUEST)
        {
            Task.Run(() =>
            {
                try
                {
                    ...
                    {
                        string[] lines = System.IO.File.ReadAllLines(tempFile);
                        string questIdRaw = lines.Length > 0 ? lines[0].Trim() : "";
                        string itemIdRaw = lines.Length > 1 ? lines[1].Trim() : "";

                        if (int.TryParse(questIdRaw, out int questId) && questId > 0)
                        {
                            int rewardId = int.TryParse(itemIdRaw, out int parsedItemId) ? parsedItemId : -1;
                            var quest = Ioc.Default.GetRequiredService<Skua.Core.Interfaces.IScriptQuest>();
                            quest.RegisterQuests((questId, rewardId));
                        }
                    }
                }
                catch { }
